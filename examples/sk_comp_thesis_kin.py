...
def generatorSKThesisKin(**kwargs) -> RKWrapper:
    ...
    rk.addCustomDerivation(
        "lbcRightExt", sc.distScalingExtrapolationDerivation(True, True)
    )
    ...
    rk.addCustomDerivation(
        "condHeatflux",
        sc.additiveDerivation(
            ["identityDeriv", "convHeatflux"], 1.0, [[1], [2, 3]], [1.0, -1.0]
        ),
    )
    ...
    delta = (1 + 65 * sqrt2 / 32 + 433 * sqrt2 / 288 - 23 * sqrt2 / 16) * ionZ + (
        5629 / 1152 - 529 / 128
    ...
    kappaDeriv = sc.simpleDerivation(
        -elCondConst * nConstGradT * normalizationConst, [2.5, -1.0]
    )
    rk.addCustomDerivation("kappa", kappaDeriv)

    qDeriv = sc.multiplicativeDerivation(
        innerDerivation="kappa",
        innerDerivationIndices=[1, 2],
        outerDerivation="gradDeriv",
        outerDerivationIndices=[1],
    )
    ...
    rk.addSpecies(
        ...
        atomicA=elMass / amu,
        charge=-1.0,
        associatedVars=["ne", "ne_dual", "Ge", "We"],
    )
    rk.addSpecies(
        "D+", -1, atomicA=2.014, charge=1.0, associatedVars=["ni", "ni_dual", "Gi"]
    )
    ...
    for neutral in neutralDensList:
        rk.addSpecies(
            ...
            int(neutral[1:]),
            heavySpeciesMass,
            associatedVars=[neutral, neutral + "_dual"],
        )
    ...
    for i in range(gridObj.numX()):
        f[i, gridObj.getH(0) - 1, :] = (
            n[i] * f[i, gridObj.getH(0) - 1, :] / numerical_dens[i]
        )
    ...
    rk.addVar(
        ...
        units="$10eV$",
        isDerived=True,
        derivationRule=sc.derivationRule("energyMoment", ["f"]),
        isCommunicated=True,
    )
    rk.addVarAndDual(
        "ne",
        ...
        units="$10^{19} m^{-3}$",
        isDerived=True,
        derivationRule=sc.derivationRule("densityMoment", ["f"]),
        isCommunicated=True,
    )
    rk.addVarAndDual(
        "Ge",
        ...
        derivationRule=sc.derivationRule("fluxMoment", ["f"]),
        isCommunicated=True,
    )
    rk.addVarAndDual(
        "qe_tot",
        isDerived=True,
        primaryOnDualGrid=True,
        derivationRule=sc.derivationRule("heatFluxMoment", ["f"]),
    )
    rk.addVarAndDual(
        "qe",
        isDerived=True,
        ...
        derivationRule=sc.derivationRule(
            "condHeatflux", ["qe_tot_dual", "Te_dual", "ue_dual"]
        ),
    )
    rk.addVarAndDual(
        "Te",
        ...
        units="$10eV$",
        isCommunicated=True,
        derivationRule=sc.derivationRule("tempFromEnergye", ["We", "ne", "Ge"]),
    )
    rk.addVarAndDual(
        "qT",
        isDerived=True,
    ...
    if loglinExtrap:
        ...
        rk.addVar(
            ...
            isDerived=True,
            isScalar=True,
            derivationRule=sc.derivationRule("logLinExtrapRight", ["ne"]),
            isCommunicated=True,
            hostScalarProcess=numProcs - numProcsH,
        )
