...
def eeCollHigherLNew(
    ...
    elDensity: Variable,
    evolvedHarmonics: List[int],
    mbLagEveryNSteps: int = 5,
    freezeMBWithinNonlinear: bool = False,
    lowPriorityMB: bool = True,
    mbLowPriorityValue: int = 5,
) -> mc.Model:
    """Return e-e collision model for l>0 using matrix terms. Assumes default
    normalization. f_l is the implicit variable throughout; Shkarofsky integrals
    enter as explicit modelbound row coefficients.

    NOTE: Must be called separately for odd and even harmonics if staggered,
    and the fluid quantities used must reflect that (dual for odd harmonics).
    """

    assert 1 not in evolvedHarmonics, \
        "eeCollHigherLNew cannot be used to evolve harmonic with index 1"
    assert mbLagEveryNSteps >= 1, "mbLagEveryNSteps must be >= 1"

    vGrid = grid.vGrid
    lNums = grid.lGrid

    # ------------------------------------------------------------------
    # Modelbound update policy
    # Uses Variable.priority directly — the only supported public API
    # in ReMKiT1D v2.0.0 for controlling modelbound update order.
    # ------------------------------------------------------------------
    def _apply_mb_policy(var: Variable) -> Variable:
        """
        Sets modelbound variable update priority by writing directly to the
        backing attribute. Variable.priority is a read-only property in this
        version of RMK_support — there is no public setter.
        """
        if lowPriorityMB:
            var.__priority__ = mbLowPriorityValue
        return var

    def _add_mb(var: Variable) -> None:
        mbData.addVar(_apply_mb_policy(var))

    # ------------------------------------------------------------------
    # Modelbound data — Shkarofsky integrals of f0 (explicit coefficients)
    # ------------------------------------------------------------------
    mbData = mc.VarlikeModelboundData()
    derivDistribution = distribution.dual if elDensity.isOnDualGrid else distribution

    _add_mb(derivations.HarmonicExtractorDerivation("f0", grid, 1)(derivDistribution))

    _add_mb(
        Variable(
            "I0", grid, isSingleHarmonic=True,
            derivation=derivations.shkarofskyIIntegralDeriv("I0", 0),
            derivationArgs=["f0"],
        ).onDualGrid(elDensity.isOnDualGrid)
    )
    ...
    _add_mb(
        Variable(
            "J-1", grid, isSingleHarmonic=True,
            derivation=derivations.shkarofskyJIntegralDeriv("J-1", -1),
            derivationArgs=["f0"],
        ).onDualGrid(elDensity.isOnDualGrid)
    )
    ...
    _add_mb(textbook["logLee"](elTemperature, elDensity))
    ...
    _add_mb(df0(derivDistribution).onDualGrid(elDensity.isOnDualGrid).rename("df0"))
    _add_mb(ddf0(derivDistribution).onDualGrid(elDensity.isOnDualGrid).rename("ddf0"))
    ...
    n_t_v3 = norms["density"] * norms["time"] / norms["velGrid"] ** 3
    c8pi   = 8 * np.pi * gamma0norm * n_t_v3
    c13    = gamma0norm / 3 * n_t_v3

    # ------------------------------------------------------------------
    # Velocity profiles — Profile objects, left-multiplied into stencils
    # NOT stored as named modelbound variables
    # ------------------------------------------------------------------
    v1 = grid.profile(np.array([1.0 / v       for v in vGrid]), "V", "\\frac{1}{v}")
    v2 = grid.profile(np.array([1.0 / v**2    for v in vGrid]), "V", "\\frac{1}{v^2}")
    v3 = grid.profile(np.array([1.0 / v**3    for v in vGrid]), "V", "\\frac{1}{v^3}")

    diag = mc.DiagonalStencil(evolvedHarmonics=evolvedHarmonics)

    # ------------------------------------------------------------------
    # Term Group 1: 8*pi * f0 * logLee * f_l  (diagonal, f_l implicit)
    # ------------------------------------------------------------------
    eeModel.ddt[distribution] += (
        c8pi * (mbData["f0"] * mbData["logLee"])
        @ diag(distribution).rename("8pif0fl")
    )

    # ------------------------------------------------------------------
    # Per-harmonic stencil cache
    # ------------------------------------------------------------------
    harm_cache = {}
    for harmonic in evolvedHarmonics:
        l = lNums[harmonic - 1]
        harm_cache[harmonic] = {
            ...
            "velDiff": stencils.D2DV2Stencil(
                harmonic, harmonic,
                adfAtZero=(0, 0) if l > 1 else (1 / vGrid[1], 0)
            ),
            "ddv":     stencils.DDVStencil(harmonic, harmonic),
        }

    # ------------------------------------------------------------------
    # Term Group 2: f_l diffusion terms via I2, J-1, IJSum
    # Outside the C1-C6 loop — one set of terms for all harmonics
    # ------------------------------------------------------------------
    for harmonic in evolvedHarmonics:
        velDiff = harm_cache[harmonic]["velDiff"]
        ddv     = harm_cache[harmonic]["ddv"]

        eeModel.ddt[distribution] += (
            c13 * (mbData["logLee"] * mbData["I2"])
            @ (v1 * velDiff(distribution)).rename("diffTermI" + str(harmonic))
        )
        eeModel.ddt[distribution] += (
            c13 * (mbData["logLee"] * mbData["J-1"])
            @ (v1 * velDiff(distribution)).rename("diffTermJ" + str(harmonic))
        )
        eeModel.ddt[distribution] += (
            c13 * (mbData["logLee"] * mbData["IJSum"])
            @ (v2 * ddv(distribution)).rename("dfdv" + str(harmonic))
        )

    # ------------------------------------------------------------------
    # Term Group 3: -l(l+1)/2 * IJSum * v3 * f_l  (diagonal)
    # Added ONCE outside all loops — h profile encodes all l values
    # ------------------------------------------------------------------
    hProfile = [lNum * (lNum + 1.0) / 2.0 for lNum in lNums]
    h = grid.profile(np.array(hProfile), "H", "\\frac{l(l+1)}{2}")
    eeModel.ddt[distribution] += (
        (-c13) * (mbData["logLee"] * mbData["IJSum"])
        @ (h * (v3 * diag(distribution))).rename("termLL")
    ...
    # ------------------------------------------------------------------
    # Term Group 4: I/J(f_l) C1-C6 terms — per harmonic
    # f_l is the implicit variable via D2DV2Stencil / DDVStencil
    # Shkarofsky integrals enter as explicit modelbound row coefficients
    # ------------------------------------------------------------------
    for harmonic in evolvedHarmonics:
        l = harm_cache[harmonic]["l"]

        # ---- Register per-l Shkarofsky integrals lazily ----
        il2Name = "I" + str(l + 2)
        if il2Name not in mbData.varNames:
            _add_mb(
                Variable(
                    il2Name, grid, isSingleHarmonic=True,
                    derivation=derivations.shkarofskyIIntegralDeriv(il2Name, l + 2),
                    derivationArgs=["f0"],
                ).onDualGrid(elDensity.isOnDualGrid)
            )

        ilName = "I" + str(l)
        if ilName not in mbData.varNames:
            _add_mb(
                Variable(
                    ilName, grid, isSingleHarmonic=True,
                    derivation=derivations.shkarofskyIIntegralDeriv(ilName, l),
                    derivationArgs=["f0"],
                ).onDualGrid(elDensity.isOnDualGrid)
            )

        jl1Name = "J" + str(-l - 1)
        if jl1Name not in mbData.varNames:
            _add_mb(
                Variable(
                    jl1Name, grid, isSingleHarmonic=True,
                    derivation=derivations.shkarofskyJIntegralDeriv(jl1Name, -l - 1),
                    derivationArgs=["f0"],
                ).onDualGrid(elDensity.isOnDualGrid)
            )

        j1lName = "J" + str(1 - l)
        if j1lName not in mbData.varNames:
            _add_mb(
                Variable(
                    j1lName, grid, isSingleHarmonic=True,
                    derivation=derivations.shkarofskyJIntegralDeriv(j1lName, 1 - l),
                    derivationArgs=["f0"],
                ).onDualGrid(elDensity.isOnDualGrid)
            )

        # ---- Stencils: f_l is the implicit variable ----
        d2fl_stencil = stencils.D2DV2Stencil(harmonic, harmonic)
        dfl_stencil  = stencils.DDVStencil(harmonic, harmonic)

        # ---- Coefficients ----
        C1 = (l + 1) * (l + 2) / ((2 * l + 1) * (2 * l + 3))
        C2 = -(l - 1) * l      / ((2 * l + 1) * (2 * l - 1))
        nc_d2_C1 = C1 * gamma0norm / 2 * n_t_v3
        nc_d2_C2 = C2 * gamma0norm / 2 * n_t_v3
        ...
        # ---- C1, C2: d²f_l/dv² with v1 profile ----
        eeModel.ddt[distribution] += (
            nc_d2_C1 * (mbData["logLee"] * mbData[il2Name])
            @ (v1 * d2fl_stencil(distribution)).rename("C1Il+2_h=" + str(harmonic))
        )
        eeModel.ddt[distribution] += (
            nc_d2_C1 * (mbData["logLee"] * mbData[jl1Name])
            @ (v1 * d2fl_stencil(distribution)).rename("C1J-l-1_h=" + str(harmonic))
        )
        eeModel.ddt[distribution] += (
            nc_d2_C2 * (mbData["logLee"] * mbData[ilName])
            @ (v1 * d2fl_stencil(distribution)).rename("C2Il_h=" + str(harmonic))
        )
        eeModel.ddt[distribution] += (
            nc_d2_C2 * (mbData["logLee"] * mbData[j1lName])
            @ (v1 * d2fl_stencil(distribution)).rename("C2J1-l_h=" + str(harmonic))
        )

        # ---- C3-C6: df_l/dv with v2 profile ----
        eeModel.ddt[distribution] += (
            (C3 * gamma0norm * n_t_v3) * (mbData["logLee"] * mbData[il2Name])
            @ (v2 * dfl_stencil(distribution)).rename("C3Il+2_h=" + str(harmonic))
        )
        eeModel.ddt[distribution] += (
            (C4 * gamma0norm * n_t_v3) * (mbData["logLee"] * mbData[jl1Name])
            @ (v2 * dfl_stencil(distribution)).rename("C4J-l-1_h=" + str(harmonic))
        )
        eeModel.ddt[distribution] += (
            (C5 * gamma0norm * n_t_v3) * (mbData["logLee"] * mbData[ilName])
            @ (v2 * dfl_stencil(distribution)).rename("C5Il_h=" + str(harmonic))
        )
        eeModel.ddt[distribution] += (
            (C6 * gamma0norm * n_t_v3) * (mbData["logLee"] * mbData[j1lName])
            @ (v2 * dfl_stencil(distribution)).rename("C6J1-l_h=" + str(harmonic))
        )
