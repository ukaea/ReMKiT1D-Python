...
import warnings
from .derivations import Derivation, NodeDerivation, DerivationArgument, InterpolationDerivation, DerivationContainer, Textbook
...
class MultiplicativeArgument:
    ...
    def __rmul__(self,lhs:Union[float,int]) -> Self:
        assert isinstance(lhs,(float,int)), "MultiplicativeArgument can only be left-multiplied by a float or int"
    ...
    def latex(self,latexRemap:Dict[str,str]={}) -> str: 
        ...
        for key in self.argMultiplicity:
            var = latexRemap[key] if key in latexRemap else "\\text{"+key.replace("_","\_")+"}"
            if self.argMultiplicity[key] > 0:
                 if isclose(self.argMultiplicity[key],1.0,rel_tol=1e-4):
                    numerator+=" "+var
                 else: 
                    power =f"{round(self.argMultiplicity[key])}" if isclose(self.argMultiplicity[key],round(self.argMultiplicity[key]),rel_tol=1e-2) else  f"{self.argMultiplicity[key]:.2f}"
                    numerator+=" "+var+"^{"+power+"}"
            else:
                if isclose(self.argMultiplicity[key],-1.0,rel_tol=1e-4):
                    denominator+=" "+var
...
class Variable(DerivationArgument):
    def __init__(self, name: str, gridObj: Grid, **kwargs) -> None:
        ...
        self.__derivation__: Union[Type[Derivation],None] = kwargs.get("derivation",None)
        ...
        if self.__derivation__ is not None:
            ...
            if not kwargs.get("isDerived", True):
                warnings.warn("Variable "+name+" has derivation rule set, but is explicitly set to not be derived. Overriding to isDerived=True!")

        self.__derivationArgs__: List[str] = kwargs.get("derivationArgs",[])
        if not self.__isDerived__ and "derivationArgs" in kwargs:
            warnings.warn("derivationArgs set for variable "+name+" which is not derived. Ignoring...")

        if self.__derivation__ is not None: 
            self.__derivationArgs__ = self.__derivation__.fillArgs(*tuple(self.__derivationArgs__))
        if isinstance(self.__derivation__,NodeDerivation):
            self.__derivationArgs__ = ct.getLeafVars(cast(NodeDerivation,self.__derivation__).node) 
            if "derivationArgs" in kwargs:
                warnings.warn("derivationArgs set for variable "+name+" which is produced by a NodeDerivation. Ignoring in favour of node leaf variables.")
        ...
        assert not(self.__isDistribution__ and self.__isSingleHarmonic__), "Variable " +name+" cannot be both full distribution and single harmonic"
        ...
        self.__dims__: Union[List[str], None] = ["x"] if not self.isOnDualGrid else ["x_dual"]
        ...
        if self.__timeDimSize__ > 0:
            dataShape = [self.__timeDimSize__] + dataShape if self.__dims__ is not None else [self.__timeDimSize__]
        ...
        self.__derivRule__: Union[Dict[Any, Any], str] = "none" if self.__derivation__ is None else {"ruleName": self.__derivation__.name, "requiredVarNames": self.__derivationArgs__}
    ...
    def values(self,data:np.ndarray):
        assert self.__data__.shape == data.shape, "data passed to values setter on Variable does not conform with dimensions"
    ...
    def addTimeDim(self,data:np.ndarray):
        ...
        assert self.__data__.shape == (1,) and len(data.shape)==1 or self.__data__.shape == data.shape[1:], "data passed to addTimeDim does not conform with dimensions"
        self.__dims__ = ["t"]+self.__dims__ if self.__dims__ is not None else ["t"]
        self.__data__ = data
    ...
    @property 
    def isFluid(self):
        return not self.__isDistribution__ and not self.__isScalar__ and not self.__isSingleHarmonic__
    ...
    def scalarHostProcess(self,proc:int):
        assert self.isScalar, "Attempted to set scalarHostProcess on non-scalar variable "+self.name
    ...
    def apply(cls,deriv:Type[Derivation],*args:Self) -> Self:
        ...
        options = {"isDistribution":args[0].isDistribution,
        ...
        return Variable(deriv.name,args[0].grid,derivation=deriv,derivationArgs = derivationArgs,**options)

    def latex(self,latexRemap:Dict[str,str] ={}):

        result = "\\text{"+self.name.replace("_","\_")+"}" if self.name not in latexRemap else latexRemap[self.name]
    ...
    def __mul__(self,rhs:Union[Self,MultiplicativeArgument,float,int]) -> MultiplicativeArgument:
        if isinstance(rhs,MultiplicativeArgument):
            return rhs*self
    ...
    def __rmul__(self,lhs:Union[float,int]) -> MultiplicativeArgument:
        assert isinstance(lhs,(int,float)), "Variable can only be left-multiplied by floats or ints"
        return lhs*MultiplicativeArgument((self,1.0))

    def __matmul__(self,rhs):
        return rhs@self

    def __truediv__(self, rhs:Union[Self,MultiplicativeArgument,float,int]) -> MultiplicativeArgument:
        if isinstance(rhs,MultiplicativeArgument):
            return (rhs**(-1))*self
    ...
    def evaluate(self,dataset:xr.Dataset) -> np.ndarray:
        if self.isDerived:
            self.__data__ = self.derivation.evaluate(*tuple(dataset[name].data for name in self.__derivationArgs__))
        return self.data
...
def varFromNode(name:str, gridObj: Grid, node: ct.Node,container:Union[DerivationContainer,None]=None,**kwargs) -> Variable: 
    ...
    return Variable(name,gridObj,derivation=NodeDerivation(name,node,container=container),**kwargs)


def varAndDual(name:str, gridObj: Grid, primaryOnDualGrid = False, dualSuffix = "_dual",**kwargs) -> Tuple[Variable,Variable]:
    ...
    """
    assert "isOnDualGrid" not in kwargs, "isOnDualGrid is not a valid kwarg for varAndDual"
    ...
    secondaryDeriv = InterpolationDerivation(gridObj,True if isDistribution else not primaryOnDualGrid,isDistribution)
    
    primary = Variable(primaryName,gridObj,isOnDualGrid=primaryOnDualGrid or isDistribution,**kwargs)

    kwargs["derivation"] = secondaryDeriv
    kwargs["derivationArgs"] = [primaryName]
    kwargs["isStationary"] = False

    secondary = Variable(secondaryName,gridObj,isOnDualGrid=not primaryOnDualGrid or isDistribution,**kwargs)
...
class VariableContainer:
    ...
    def __init__(self, gridObj: Grid, timestamps: np.ndarray = np.array([])) -> None:
        ...
        """
        self.__coords__ = {"x": gridObj.xGrid, "x_dual": gridObj.xGridDual, "h": range(gridObj.numH()), "v": gridObj.vGrid}
        if len(timestamps) > 0 :
            self.__coords__["t"] = timestamps
        self.__dataset__ = xr.Dataset(
        ...
        self.__coordUnits__ = {self.__dataset__.coords[coord].attrs["units"] for coord in self.__dataset__.coords if "units" in self.__dataset__.coords[coord].attrs}
        
        self.__grid__ = gridObj
        self.__variables__:List[Variable] = [Variable("time",gridObj,isDerived=True,isScalar=True)]

    def add(self, *args: Variable):
        """Add any number of variables to the container
        """
        for var in args:
            assert not var.isSingleHarmonic, "Single harmonic variable "+var.name+"in add - VariableContainers do not support single harmonics"
            
            if var.name in self.varNames:
                warnings.warn("Variable "+var.name+" already in VariableContainer. Overwriting.")
    ...
    def dict(self, outputVals=True) -> dict:
        ...
        for var in implicitVars:
            ...
            if outputVals:
                varProps["initVals"] = (
                    ds.data_vars[var].data.flatten().tolist()
                )
        ...
        for var in derivedVars:
            ...
            if ds.data_vars[var].attrs["derivationRule"] != "none":
                varProps["derivationRule"] = ds.data_vars[var].attrs[
                    "derivationRule"
                ]
...
class MPIContext:
    ...
    def addToComm(
        self, var:Variable, mpiComm = Dict[str,object]
    ) -> None:

        if var.isDistribution:
            cast(
                List[str],
                cast(Dict[str, object], mpiComm)[
                    "varsToBroadcast"
                ],
            ).append(var.name)
            cast(
            ...
            assert var.scalarHostProcess <= self.numProcs, "Variable "+var.name+" has scalar host process out of MPI rank bounds"
            cast(
                List[str],
                cast(Dict[str, object], mpiComm)[
    ...
    def dict(self,varCont:VariableContainer) -> dict:
        ...
        mpiDict = {"numProcsX": self.numProcsX,
