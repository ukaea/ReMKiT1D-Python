...
class StaggeredDivStencil(Stencil):
    ...
    def __init__(self):
        super().__init__(latexTemplate="\\nabla\\cdot\\left($0\\right)",properties={"stencilType": "staggeredDifferenceStencil"})

class StaggeredGradStencil(Stencil):

    def __init__(self):
        super().__init__(latexTemplate="\\nabla\\left($0\\right)",properties={"stencilType": "staggeredDifferenceStencil","ignoreJacobian": True})

class BCDivStencil(Stencil):

    def __init__(self, fluxJac:Variable, lowerBoundVar: Union[Variable,None]=None,isLeft:bool=False):
        ...
        assert not fluxJac.isOnDualGrid, "Flux Jacobian variable for BCDivStencil must live on the regular grid"
    ...
    def latex(self, arg:MultiplicativeArgument, **kwargs):
        ...
        if self.__lowerBoundVar__ is not None: 
            boundVar += ", "
            boundVar += latexRemap[self.__lowerBoundVar__.name] if self.__lowerBoundVar__.name in latexRemap else "\\text{"+self.__lowerBoundVar__.name.replace("_","\_")+"}"
        fluxJac =  latexRemap[self.__fluxJac__.name] if self.__fluxJac__.name in latexRemap else "\\text{"+self.__fluxJac__.name.replace("_","\_")+"}"
        return "\\nabla_{BC,"+LR+boundVar+"}\\cdot\\left("+fluxJac+" "+arg.latex(latexRemap)+"\\right)"
...
class CentralDiffDivStencil(Stencil):
    ...
    def latex(self, arg:MultiplicativeArgument, **kwargs):
        ...
        if self.__fluxJac__ is not None:
            fluxJac =  latexRemap[self.__fluxJac__.name] if self.__fluxJac__.name in latexRemap else "\\text{"+self.__fluxJac__.name.replace("_","\_")+"}"
        return "\\nabla_{c}\\cdot\\left("+fluxJac+" "+arg.latex(latexRemap)+"\\right)"

class CentralDiffGradStencil(Stencil):

    def __init__(self):
            super().__init__(latexTemplate="\\nabla_{c}\\left($0\\right)",properties= {"stencilType": "centralDifferenceInterpolated", "ignoreJacobian": True}
    )

class DiffusionStencil(Stencil):

    def __init__(self, deriv:DerivationClosure,diffCoeffOnDualGrid=False,ignoreJacobian=False):
        assert deriv.numArgs == 0, "deriv in DiffusionStencil must be a complete closure"
...
class MomentStencil(Stencil):
    ...
    def __init__(self, momentOrder:int,momentHarmonic:int):
        super().__init__("\\langle $0 \\rangle_{h="+str(momentHarmonic)+",n="+str(momentOrder)+"}",
                         {
...
class DistGradStencil(Stencil):
    ...
    def __init__(self, rowHarmonic:str,colHarmonic:str):
        super().__init__("\\delta_{h,"+str(rowHarmonic)+"}\\nabla \\left( $0_{h'="+str(colHarmonic)+"}\\right)", {
...
class DDVStencil(Stencil):
    ...
    def __init__(self, rowHarmonic:int, colHarmonic:int,CCoeff:Union[None,Profile,Variable]=None,interpCoeffs:Union[None,Profile,Variable]=None,cfAtZero:Union[Tuple[float,float],None]=None):
        stencil = {
            "stencilType": "ddvStencil",
            "modelboundC": "none" if not isinstance(CCoeff,Variable) else CCoeff.name,
            "modelboundInterp": "none" if not isinstance(interpCoeffs,Variable) else interpCoeffs.name,
            "rowHarmonic": rowHarmonic,
            "colHarmonic": colHarmonic,
        }
        ...
        if CCoeff is None:
            latexTemplate = "\\frac{\\partial $0_{h'="+str(colHarmonic)+"}}{\\partial v}"
        else:
            C = CCoeff.latex() if isinstance(CCoeff,Profile) else "C_{MB}"
            latexTemplate = "\\frac{\\partial}{\\partial v}\\left("+C+"$0_{h'="+str(colHarmonic)+"}\\right)"
...
class D2DV2Stencil(Stencil):
    ...
    def __init__(self, rowHarmonic:int, colHarmonic:int,diffCoeff:Union[None,Profile,Variable]=None,adfAtZero:Union[Tuple[float,float],None]=None):
        stencil = {
           "stencilType": "vDiffusionStencil",
            "modelboundA": "none" if not isinstance(diffCoeff,Variable) else diffCoeff.name,
            "rowHarmonic": rowHarmonic,
            "colHarmonic": colHarmonic,
        }
        ...
        if diffCoeff is None:
            latexTemplate = "\\frac{\\partial^2 $0_{h="+str(colHarmonic)+"}}{\\partial v^2}"
        else:
            A = diffCoeff.latex() if isinstance(diffCoeff,Profile) else "A_{MB}"
            latexTemplate = "\\frac{\\partial}{\\partial v}\\left("+A+"\\frac{\\partial $0_{h'="+str(colHarmonic)+"}}{\\partial v}\\right)"
        latexTemplate = "\\delta_{h,"+str(rowHarmonic)+"}"+latexTemplate
        super().__init__(latexTemplate, properties=stencil)
...
class ShkarofskyIStencil(Stencil):
    ...
    def __init__(self, rowHarmonic: int, colHarmonic: int, integralIndex: int):
        super().__init__("\\delta_{h,"+str(rowHarmonic)+"}I_"+str(integralIndex)+"\\left($0_{h'="+str(colHarmonic)+"}\\right)", 
                         {
...
class ShkarofskyJStencil(Stencil):
    ...
    def __init__(self, rowHarmonic: int, colHarmonic: int, integralIndex: int):
        super().__init__("\\delta_{h,"+str(rowHarmonic)+"}J_"+str(integralIndex)+"\\left($0_{h'="+str(colHarmonic)+"}\\right)", 
                         {
        "stencilType": "shkarofskyIJStencil",
        "JIntegral": True,
        "rowHarmonic": rowHarmonic,
...
class TermMomentStencil(Stencil):
    ...
    def __init__(self,colHarmonic:int, momentOrder:int,termName:str):
        super().__init__("\\langle \\left( \\frac{\\partial $0_{h="+str(colHarmonic)+"}}{\\partial t} \\right)_{\\text{"+termName.replace("_","\_")+"}} \\rangle_{n="+str(momentOrder)+"}",
                         {
...
class FixedEnergyBoltzmannStencil(Stencil):
    ...
    def __init__(self, rowHarmonic: int,
                    transitionIndex: int,
        ...
        transition = transitionLatex if transitionLatex is not None else "T="+str(transitionIndex)
        super().__init__("\\delta_{h,"+str(rowHarmonic)+"}C^"+AE+"_{"+transition+"}\\left($0_h\\right)", {
...
class LBCStencil(Stencil):
    ...
    def __init__(self, rowHarmonic: int,
        ...
        if densityVarOnBoundary is not None:
            assert densityVarOnBoundary.isScalar, "LBC boundary density variable must be a scalar"
        if densityVarDual is not None:
            assert densityVarDual.isOnDualGrid, "Dual density variable in LBC stencil must live on dual grid"
        ...
        super().__init__(latexTemplate="\\delta_{h,"+str(rowHarmonic)+"}\\nabla_{LBC,"+("L"if leftBoundary else "R")+"}\\left($0_{h'="+str(colHarmonic)+("" if decompHarmonics is None else "{,\\text{partial}}")+"}\\right)", properties=stencil)
...
class CustomFluid1DStencil(Stencil):
    ...
    def __init__(self, xStencil: Tuple[int,...],
        ...
        if varContColumnVars is not None:
            ...
            stencil.update({"columnVarContVars": [var.name if var is not None else "none" for var in varContColumnVars]})
        else:
        ...
        if mbDataColumnVars is not None:
            ...
            stencil.update({"columnMBDataVars": [var.name if var is not None else "none" for var in mbDataColumnVars]})
        else:
            stencil.update({"columnMBDataVars": ["none" for _ in xStencil]})
        
        super().__init__(latexTemplate if latexTemplate is not None else "\\text{CustomStencil}\\left($0\\right)", properties=stencil)